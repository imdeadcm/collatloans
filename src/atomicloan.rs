...
use crate::cves::{precompute_enc_cs, enc_cs_with_precomputation, CVESCiphertext};
use rayon::prelude::*;
...
pub fn prepare_loan(gamma: usize, installments:usize, pk:G1Affine, bank_kp:&SchnorrPair) -> Vec<CVESCiphertext> {
    ...
    let loan_ciphertexts: Vec<CVESCiphertext> = conditions
        ...
        .map(| condition| {
            ...
            if condition.j == 0{
                let c_ves = enc_cs_with_precomputation(gamma.clone(), pk, condition.witness.clone(), &condition.transition, w0.clone(), &bank_kp, &condition.state, &precom);
                c_ves
